...
def evaluate_segment(input_data: dict, expected_output: dict, conn) -> dict:
    ...
    # DEBUG — print first offer's extracted fields
    first_offer = result["offers_parsed"][0] if result["offers_parsed"] else None
    if first_offer:
        print(f"\nDEBUG — Segment: {input_data['segment']}")
        print(f"Offer: {first_offer['id']}")
        na_count = sum(1 for v in first_offer['fields'].values() if v == 'N/A')
        total_count = len(first_offer['fields'])
        print(f"N/A fields: {na_count}/{total_count}")
        print("Sample fields (first 10):")
        for i, (k, v) in enumerate(list(first_offer['fields'].items())[:10]):
            print(f"  {k}: {repr(v)}")
