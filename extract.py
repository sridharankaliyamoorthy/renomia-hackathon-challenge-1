...
from normalize import is_conditions_doc, is_quotation_doc, clean_ocr_text
...
_SEGMENT_CONTEXT = {
    ...
    "lodě": (
        ...
        "Look for: hull premium, TPL/liability limit, deductible, total annual cost.\n"
        "Documents may be in English, Czech, or Polish.\n"
        "If Option A / Option B shown, extract the HIGHER coverage option."
    ),
}
...
def extract_offer(
    ...
    for doc in offer.get("documents", []):
        ...
        if len(ocr_text) < 50 and doc.get("pdf_url"):
    ...
    # Step 8 — Auta-specific postprocessing
    if segment == "auta":
